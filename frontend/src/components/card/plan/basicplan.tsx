...
export class Plan extends React.Component<NoticeProps>{
    ...
    free: plan = {
        price: 0,
        list: ["Pay only for hyped releases"],
        checklist: [""],
        primary: false
    }
    ...
    PlanCard = styled(Card)<{primary?: boolean}>`
        background-color: ${props => props.primary ? "#1A1B1C" : "#242629"};
        color: white;
    `;
    PlanButton = styled(Button)<{primary?: boolean}>` 
        width: 50%;
        background-color: ${props => props.primary ? "#1A1B1C" : "#7F5AF0"};
        color: white;
        font-weight: 700;
    `;
    PlanText = styled(Typography)<{size?: number}>`
        ...
        font-weight: bold;
    `;
    PlanInfo = styled(Typography)<{size?: number}>`
    padding:1rem;
    display: inline;
`;
    getPlan() {
                ...
                <PlanCard primary={plan.primary}>
                    <CardContent>
                        <Grid container direction="column" justify="center" alignItems="center">
                            <Title size={2.5} variant="h5" component="h5">{this.getPlan()}</Title>
                            <PlanInfo>
                                {plan.list.map((item: string, index:number) => this.getListElement({item, index}) )}
                            </PlanInfo>
                            <PlanButton variant="contained" primary={plan.primary}>{/*{this.props.title}*/}COMING SOON</PlanButton>
                        </Grid>
                    </CardContent>
                </PlanCard>
            </Grid>
